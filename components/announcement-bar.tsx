        ...
        <div ref={scrollRef} className="flex gap-6 items-center w-max px-4">
          ...
          {[...announcements, ...announcements].map((item, index) => (
            <div key={index} className="flex items-center gap-2 whitespace-nowrap">
              ...
              {item.type === "cta" && (
                <>
                  <span className="text-[#06ff83] text-xs">{item.icon}</span>
                  <a
                    href={"link" in item ? item.link : "#"}
                    className="text-xs font-medium text-[#06ff83] hover:text-[#47ffa4] underline cursor-pointer transition-colors"
                  >
                    {item.text}
                  </a>
                </>
